...
export default function AddQuiz() {
  ...
  const handleAddQuestion = () => {
    setQuestions([
      ...questions,
      { question: '', options: {}, fields: [], type: 'single' },
    ]);
  };
  ...
  const handleAddField = (questionIndex) => {
    const updatedQuestions = [...questions];
    updatedQuestions[questionIndex].fields.push({
      key: `field${Date.now()}`,
      label: '',
      correct: '',
    });
  ...
  if (!isAdmin) return <p>Ładowanie...</p>;
        ...
        {questions.map((q, questionIndex) => (
          <div key={questionIndex} className="border p-4 rounded mb-4">
            <input
              ...
              value={q.question}
              onChange={(e) =>
                handleUpdateQuestion(questionIndex, 'question', e.target.value)
              }
            ...
            <select
              value={q.type}
              onChange={(e) =>
                handleUpdateQuestion(questionIndex, 'type', e.target.value)
              }
              className="w-full border mb-2 p-2"
            >
